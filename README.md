# Anubhav Gain's Security Blog

[![Built with Astro](https://img.shields.io/badge/Built%20with-Astro-BC52EE.svg)](https://astro.build)
[![Powered by gainsaheb](https://img.shields.io/badge/Powered%20by-gainsaheb-blue.svg)](https://github.com/saicaca/gainsaheb)
![Security Focus](https://img.shields.io/badge/Focus-Cybersecurity-red.svg)
![Rust](https://img.shields.io/badge/Language-Rust-orange.svg)
![Citations](https://img.shields.io/badge/Research%20Citations-222+-green.svg)

Welcome to my technical blog where I share insights on cybersecurity, DevSecOps, and advanced threat detection technologies.

## About Me

I'm **Anubhav Gain**, a distinguished cybersecurity professional and entrepreneur who has established himself as a leading figure in India's cybersecurity landscape. Currently serving as **Security Software Engineer at Infopercept Consulting** while operating as **CEO of TechAnv Consulting**.

### Current Roles

- 🏢 **Security Software Engineer** at Infopercept Consulting (July 2024 - Present)
  - Lead architect of the **Invinsense XDR/OXDR cybersecurity platform**
  - Design single-tenant and multi-tenant security deployments
  - Develop custom OpenSearch Dashboard plugins
  - Create Rust-based cross-platform monitoring tools

- 👔 **CEO & Owner** at TechAnv Consulting (December 2022 - Present)
  - Cybersecurity research and consulting
  - Security architecture recommendations
  - Vulnerability assessments and threat analysis

- 🔧 **DevSecOps Engineer** at Atcults (November 2023 - Present)
  - Automated security testing pipelines
  - Infrastructure as Code with Terraform
  - Container orchestration with Docker and Kubernetes

- 🐟 **Owner** at Gain Fisheries (August 2020 - Present)
  - Sustainable aquaculture in Pakhanjur, Chhattisgarh
  - Environmental stewardship and innovation

## Education & Certifications

### Academic Qualifications
- **B.Tech in Cyber/Computer Forensics and Counterterrorism** - Parul University (2021-2025)
- **Licentiate in Cybersecurity Management** - Charles Sturt University (High Distinction: 93/100)
- **Licentiate in Ransomware Techniques** - Charles Sturt University

### Professional Certifications
- C3SA Premium Edition
- IBM Cybersecurity Analyst Professional Certificate
- AWS Educate Getting Started with Security
- Intel Solution Pro Cloud Business Professional
- Harvard CS50 Series
- Cisco Networking Basics & Introduction to Cybersecurity

## Technical Expertise

### Core Technologies
- **Languages**: Rust (Primary), Python, Go, Bash, PowerShell, YARA
- **Security Platforms**: XDR/OXDR, SIEM, EDR, SOAR
- **Cloud**: AWS, Azure, Multi-cloud architectures
- **Containers**: Docker, Kubernetes, Podman
- **Infrastructure**: Terraform, Ansible, CI/CD pipelines

### Specializations
- Extended Detection and Response (XDR/OXDR) platforms
- eBPF-based security monitoring
- DevSecOps and security automation
- Threat and vulnerability management
- Cloud security architecture
- Zero Trust implementation

## Research & Publications

- **222+ Academic Citations** on Google Scholar
- Co-authored research on **EEG-based emotion recognition using LSTM networks**
- Multiple papers in cybersecurity, automation, and system administration
- Active contributor to open source security projects

## Notable Projects

### Open Source Contributions
- **honeypot.rs**: CVE-2023-46604 Apache ActiveMQ RCE detection
- **eBPF Security Monitoring**: Kernel-level system monitoring tools
- **Rust Security Utilities**: Cross-platform security solutions
- **SOC Fortress Playbooks**: Threat detection rules
- **Wazuh Rules**: Enhanced SIEM detection capabilities

### Enterprise Collaborations
- **JPMorgan Chase & Co.**: Fraud detection and prevention strategies
- **PwC**: Phishing simulation and integrated defense
- **AIG**: Zero-day vulnerability response and ransomware defense
- **Clifford Chance**: ICO regulatory compliance

## Blog Topics

This blog covers cutting-edge security topics including:

### Platform Development
- Building the Invinsense XDR/OXDR platform
- Multi-tenant security architectures
- Real-time threat correlation engines

### Security Engineering
- eBPF kernel-level monitoring
- Rust security patterns
- Post-quantum cryptography
- Zero Trust architectures

### DevSecOps
- CI/CD security integration
- Container security
- Infrastructure as Code security
- Automated remediation

### Research & Innovation
- AI-driven threat detection
- Behavioral analytics
- Advanced persistent threat hunting
- Security automation frameworks

## Geographic Presence

- **Ahmedabad, Gujarat**: Infopercept Consulting HQ
- **Vadodara, Gujarat**: TechAnv Consulting & Parul University
- **Pakhanjur, Chhattisgarh**: Gain Fisheries

## Connect With Me

- 💼 **LinkedIn**: [in.linkedin.com/in/anubhavgain](https://in.linkedin.com/in/anubhavgain)
- 🐙 **GitHub**: [@mranv](https://github.com/mranv) & [@anubhavg-icpl](https://github.com/anubhavg-icpl)
- 📚 **Google Scholar**: [Research Profile](https://scholar.google.com/citations?user=yhcz3TkAAAAJ&hl=en)
- 🌐 **Websites**:
  - [mranv.pages.dev](https://mranv.pages.dev)
  - [mranv.github.io](https://mranv.github.io)
  - [anubhavg-icpl.github.io](https://anubhavg-icpl.github.io)
- 📷 **Instagram**: [@anubhavgain](https://www.instagram.com/anubhavgain/)

## Latest Posts

- **Building Invinsense**: The journey of creating an enterprise XDR/OXDR platform
- **My Cybersecurity Journey**: From code to defense
- **Open XDR Revolution**: Breaking vendor lock-in
- Advanced eBPF security monitoring techniques
- Rust-based security tool development

## Contributing

Feel free to:
- Open issues for blog post suggestions
- Submit corrections or improvements
- Share feedback on security topics
- Request deep dives on specific technologies

## License

Blog content is licensed under [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/).

---

*"In the world of cybersecurity, paranoia is just another word for experience."*

Building robust, scalable defenses one line of Rust at a time.
