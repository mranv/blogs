...
const accessibilityRules: AccessibilityRule[] = [
      ...
      images.forEach((img, index) => {
        ...
        if (!alt) {
          issues.push({
            ...
            suggestion:
              "Add descriptive alt text to help screen readers understand the image",
          ...
          issues.push({
            ...
            suggestion:
              'Use alt="" only for decorative images, otherwise provide descriptive text',
            wcagGuideline: "WCAG 2.1 - 1.1.1 Non-text Content (Level A)",
          });
        }
      });
      ...
      inputs.forEach((input, index) => {
        ...
        if (!hasLabel && !hasAriaLabel && !hasAriaLabelledBy) {
          issues.push({
            ...
            suggestion:
              "Add a <label> element or use aria-label/aria-labelledby",
  ...
  },
  {
    ...
    check: container => {
      ...
      const clickableDivs = container.querySelectorAll(
...
const AccessibilityTester: React.FC<AccessibilityTesterProps> = ({
  ...
  const [selectedLevel, setSelectedLevel] = useState<"A" | "AA" | "AAA">(
    wcagLevel
  );
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <select
              value={selectedLevel}
              onChange={e =>
                setSelectedLevel(e.target.value as "A" | "AA" | "AAA")
              }
      ...
      {testResult && (
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div className="grid grid-cols-4 gap-4 text-center">
            <div>
              <div
                className={`text-3xl font-bold ${getScoreColor(testResult.score)}`}
              >
                {testResult.score}
              </div>
