...
const PerformanceTester: React.FC<{
  ...
  const [currentTest, setCurrentTest] = useState<PerformanceTestResult | null>(
    null
  );
  ...
  const stopPerformanceTest = useCallback(() => {
    ...
    if (options.trackRenders && data.renderTimes.length > 0) {
      ...
      if (avgRenderTime > 16) {
        recommendations.push(
          "Consider optimizing render performance - average render time exceeds 16ms"
        );
      }
    }
    ...
    if (options.trackFPS && data.frameRates.length > 0) {
      const avgFPS =
        data.frameRates.reduce((a, b) => a + b, 0) / data.frameRates.length;
      ...
      metrics.push({
        ...
        threshold: 60,
        status:
          avgFPS >= 55 ? "good" : avgFPS >= 30 ? "needs-improvement" : "poor",
      ...
      metrics.push({
        ...
        threshold: 30,
        status:
          minFPS >= 30 ? "good" : minFPS >= 20 ? "needs-improvement" : "poor",
        description: "Lowest FPS recorded during monitoring",
      });

      if (minFPS < 30) {
        recommendations.push(
          "FPS drops below 30 - consider reducing animation complexity"
        );
      }
    }
    ...
    if (options.trackMemory && data.memoryUsage.length > 0) {
      ...
      metrics.push({
        ...
        threshold: 50,
        status:
          latestMemory.used / 1024 / 1024 < 50
            ? "good"
            : latestMemory.used / 1024 / 1024 < 100
              ? "needs-improvement"
              : "poor",
      ...
      metrics.push({
        ...
        threshold: 10,
        status:
          memoryIncrease / 1024 / 1024 < 10
            ? "good"
            : memoryIncrease / 1024 / 1024 < 25
              ? "needs-improvement"
              : "poor",
      ...
      if (memoryUsagePercent > 70) {
        recommendations.push(
          "High memory usage - consider optimizing memory consumption"
        );
      }
    }
    ...
    if (data.renderCount > 50) {
      recommendations.push(
        "High number of re-renders - consider memoization or optimization"
      );
    }
      ...
      <div className="bg-gray-50 dark:bg-gray-800 px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              onClick={
                isMonitoring ? stopPerformanceTest : startPerformanceTest
              }
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center space-x-4">
              <div className="text-right">
                <div
                  className={`text-2xl font-bold ${getScoreColor(currentTest.score)}`}
                >
                  {currentTest.score}
                </div>
          ...
          <div className="space-y-2">
            {results.slice(1).map((result, index) => (
              ...
              >
                <div className="flex items-center space-x-4">
                  <div
                    className={`text-lg font-medium ${getScoreColor(result.score)}`}
                  >
                    {result.score}
                  </div>
                  <div className="text-sm text-gray-600 dark:text-gray-400">
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400">
                  ...
                  {result.metrics.length} metrics passed
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-4">
            <div>
              ...
              <ul className="text-gray-600 dark:text-gray-400 mt-1">
                <li>
                  • Renders: {options.trackRenders ? "Enabled" : "Disabled"}
                </li>
                <li>
                  • Memory: {options.trackMemory ? "Enabled" : "Disabled"}
                </li>
                <li>• FPS: {options.trackFPS ? "Enabled" : "Disabled"}</li>
                <li>
                  • Bundle Size:{" "}
                  {options.trackBundleSize ? "Enabled" : "Disabled"}
                </li>
              </ul>
            </div>
            <div>
