...
const ThemeTester: React.FC<ThemeTesterProps> = ({
  ...
  const testTheme = async (theme: ThemeVariant): Promise<ThemeTestResult> => {
    ...
    try {
      ...
      const hasThemeClass = theme.className ? container.classList.contains(theme.className) : true;
      ...
      if (testColorContrast) {
        const textElements = container.querySelectorAll('p, span, a, button, h1, h2, h3, h4, h5, h6, li, td, th, label');
          ...
          if (color && backgroundColor && backgroundColor !== 'rgba(0, 0, 0, 0)') {
            ...
            if (contrastRatio < 4.5) {
              issues.push({
                ...
                suggestion: 'Increase color contrast to meet WCAG AA standards (4.5:1 minimum)'
              });
              ...
              issues.push({
                ...
                element: element.tagName.toLowerCase(),
                suggestion: 'Consider increasing contrast for better accessibility (7:1 for AAA)'
              });
            }
          }
        });
      ...
      const themableElements = container.querySelectorAll('[class*="bg-"], [class*="text-"], [class*="border-"]');
      ...
      themableElements.forEach((element, index) => {
        ...
        const hasHardcodedStyles = elementStyles.backgroundColor?.includes('rgb(') && 
      ...
      images.forEach((media, index) => {
        const hasThemeVariant = media.getAttribute('data-theme-light') || 
      ...
      interactiveElements.forEach((element, index) => {
        ...
        const hasHoverStates = classList.some(cls => 
          cls.includes('hover:') || cls.includes('focus:') || cls.includes('active:')
        );
        ...
        if (!hasHoverStates) {
          issues.push({
            ...
            element: element.tagName.toLowerCase(),
            suggestion: 'Add hover and focus states that work with the current theme'
          });
        }
      });
      ...
      const hasThemeVariables = rootStyles.getPropertyValue('--theme-primary') || 
      ...
      if (!hasThemeVariables) {
        issues.push({
          ...
          element: 'root',
          suggestion: 'Consider using CSS variables for better theme consistency'
        });
      }
  ...
  const runSingleThemeTest = async () => {
    ...
    setTestResults(prev => [result, ...prev.filter(r => r.theme !== currentTheme.name)]);
  };
      ...
      <div className="bg-gray-50 dark:bg-gray-800 px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            <select
              value={previewMode}
              onChange={(e) => setPreviewMode(e.target.value as 'split' | 'single')}
        ...
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div>
              ...
              <div className="text-sm text-gray-600 dark:text-gray-400">Themes Tested</div>
            </div>
            <div>
      ...
      <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
        <details className="text-sm">
          ...
          <div className="mt-3 space-y-2 text-gray-600 dark:text-gray-400">
            <p>• Color Contrast Testing: {testColorContrast ? 'Enabled' : 'Disabled'}</p>
            <p>• Responsive Testing: {testResponsiveness ? 'Enabled' : 'Disabled'}</p>
